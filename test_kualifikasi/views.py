...
def stored_procedure():
    return f"""
        create or replace function sudah_ujian()
        returns trigger as
        $$
        begin
        if exists (select*from atlet_nonkualifikasi_ujian_kualifikasi
        where id_atlet=new.id_atlet and
        tahun= new.tahun and
        batch= new.batch and
        tempat=new.tempat and
        tanggal=new.tanggal) then
        raise exception 'error';
        elseif exists(select*from atlet kualifikasi
        ...
        create or replace function jika_lulus()
        returns trigger as
        $$ 
        begin
